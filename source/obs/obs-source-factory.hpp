...
namespace obs {
	...
	class source_factory {
		...
		source_factory()
		{
			...
			set_have_active_child_sources(false);
		}
		...
		{
			if (v) {
				_info.get_width  = _width;
				_info.get_height = _height;
			} else {
		...
		try {
			...
			DLOG_ERROR("Uncaught exception in '%s'.", __FUNCTION_NAME__);
			return nullptr;
		}
		...
		static std::uint32_t _width(void* data) noexcept
		try {
			if (data)
				return reinterpret_cast<_instance*>(data)->width();
			return 0;
		} catch (const std::exception& ex) {
			DLOG_ERROR("Uncaught exception in '%s': %s.", __FUNCTION_NAME__, ex.what());
			return 0;
		...
		static std::uint32_t _height(void* data) noexcept
		try {
			if (data)
				return reinterpret_cast<_instance*>(data)->height();
			return 0;
		} catch (const std::exception& ex) {
			DLOG_ERROR("Uncaught exception in '%s': %s.", __FUNCTION_NAME__, ex.what());
			return 0;
	...
	class source_instance {
		...
		virtual std::uint32_t width()
		{
			return 0;
		}

		virtual std::uint32_t height()
		{
			return 0;
		}
